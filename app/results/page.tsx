...
import { parseExamMode, type ExamMode } from "@/lib/exam-mode";
...
function ResultsPageContent() {
  ...
  const calculateAverageScores = () => {
    ...
    return [
      {
        subject: t("유창성"),
        A: Math.round(sum.fluency / count),
        fullMark: 100,
      },
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge variant="outline">{t(mode === "practice" ? "연습" : "실전 모의고사")}</Badge>
            </div>
            <p className="text-slate-500">
              {isAnalyzing ? t("AI로 모든 답변을 분석하는 중...") : t("성적과 AI 피드백을 확인하세요.")}
            </p>
          </div>
            ...
            <CardContent className="flex items-center justify-center py-12">
              <div className="text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700">{t("잠시 시간이 걸릴 수 있습니다. 기다려 주세요.")}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <CardContent className="flex flex-col items-center">
                  <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="80%" data={calculateAverageScores()}>
                        <PolarGrid />
                        <PolarAngleAxis dataKey="subject" />
                        <PolarRadiusAxis angle={30} domain={[0, 100]} />
                        <Radar name={t("내 점수")} dataKey="A" stroke="#2563eb" fill="#3b82f6" fillOpacity={0.6} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>

                  <div className="mt-4 text-center">
                    <p className="text-sm text-slate-500 mb-1">{t("예상 등급")}</p>
                    <Badge className="text-2xl px-4 py-2 bg-blue-600 hover:bg-blue-700">{overallGrade || "—"}</Badge>
                  </div>
                </CardContent>
              </Card>
                        ...
                        return (
                          <AccordionItem key={q.id} value={`item-${q.id}`}>
                            ...
                            <AccordionContent className="space-y-4 pt-2">
                              <p className="text-sm text-slate-600 italic">&quot;{q.text}&quot;</p>
                              ...
                              {analysis ? (
                                ...
                                <div className="text-center py-4 text-slate-500">
                                  {isAnalyzing ? t("분석 중...") : t("분석 결과가 없습니다")}
                                </div>
                              )}
                            </AccordionContent>
                          </AccordionItem>
